 ...
 *
 * <p>유저·생성자 등록만 담당한다.
 * 발급된 쿠폰 조회 ({@code GET /api/users/{userId}/coupons}) 는
 * 각 issuer 모듈의 {@code UserCouponController} 에 위치한다.</p>
 */
@RestController
...
public class UserController {
    ...
    public BaseResponse<RegisterUserResponse> registerUser(
        ...
        return BaseResponse.ok(RegisterUserResponse.builder().userId(userId).build());
    }

    @PostMapping("/api/creators")
    public BaseResponse<RegisterCreatorResponse> registerCreator(
        ...
        return BaseResponse.ok(RegisterCreatorResponse.builder().creatorId(creatorId).build());
    }
}